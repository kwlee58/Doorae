...
pander(status.vs.structure.ap, digits=1, caption="ㅇ공동체상태와 조직형태(%, 계)")
```
...
* 공동체상태와 조직형태
    * 경영개선 상태에서 법인격을 갖춘 공동체사업의 비율이 창업준비 상태에서 법인격을 갖춘의 공동체사업의 비율보다 압도적으로 많다는 점에 유의.
...
```{r, echo=FALSE, results='asis'}
# kable(ftable(table("공동체상태"=doorae$"공동체상태", "조직형태"=doorae$"조직형태", "사업자등록 여부"=doorae$"성과_사업자등록")))
```
...
box.5 <- box.4 + annotate("text", x=c(1.32, 2.2), y=c(12, 12.5), label=c("아름다운인제관광", "예그리나"), size=3.5, family="AppleGothic")
box.5
```
...
ecdf.8 <- ecdf.7 + annotate("segment", x=1.3, xend=4.39, y=0.5, yend=0.5, colour="gray50", arrow=arrow(ends="both", length=unit(0.2, "cm")))
ecdf.9 <- ecdf.8 + scale_x_continuous(breaks=c(0, 1.3, 4.39, 11.6, 12.3), labels=c(0, 1.3, 4.39, 11.6, 12.3))
...
```{r, echo=FALSE}
x.pre <- tapply(doorae.ys$years, doorae.ys$status, sort)$pre
x.post <- tapply(doorae.ys$years, doorae.ys$status, sort)$post
n.pre<-length(x.pre)
...
poly.6 <- poly.5 + scale_x_continuous(breaks=c(0, 1.3, 4.39, 11.6, 12.3), labels=c(0, 1.3, 4.39, 11.6, 12.3))
